import { AccountBalance, AccountCircle, Add, Call, Delete, Home, Place } from '@mui/icons-material';
import { Box, Button, Card, Checkbox, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormHelperText, Input, InputLabel, MenuItem, Paper, Select, Table, TableBody, TableCell, TableContainer, TableFooter, TableHead, TablePagination, TableRow, TextField, Typography } from '@mui/material';
...
const Invoices = () => {
            ...
            <Paper>
                ...
                >
                    ...
                    <DialogContent>
                        ...
                        >
                            {
                                items.map((item) => {
                                    return (
                                        <MenuItem key={item.code} id={item.code} value={item.code}>{item.name}</MenuItem>
                                    )
                                })
                            }
                        </Select>
                        ...
                        >
                            {
                                !selectedUoms.length>0 && selectedUoms.map((uom) => {
                                    return ( 
                                        <MenuItem key={uom.code} id={uom.code} value={uom.code}>{uom.name}</MenuItem>
                                    )
                                })
                            }
                        </Select>
    ...
    const TraderBankDetails = () => {
        ...
        return (
            <Paper>
                <Typography variant='h4'><AccountBalance />Bank Details</Typography>
                {
                    traders.map((trader) => {
                        return (
                            <Card className={styleClasses.customerDetails} key={trader.accountNumber}>
            ...
            <div>
                {
                    ...
                    !Array.isArray(traders) || !traders.length ? null : traders.map((trader, index) => {
                        ...
                        return (
                            <Card key={trader.code} className={styleClasses.customerDetails}>
                                <Typography variant='h4'><AccountCircle />{trader.name}</Typography>
                                <span className={styleClasses.data}><Home />{trader.address}</span><br />
                                <span className={styleClasses.data}>{trader.gst}</span><br />
                                <span className={styleClasses.data}><Place />{states.length > 0 && findState(stateCode) ? findState(stateCode).name : "State not found"} </span><br />
                                <span className={styleClasses.data}><Call />{trader.contact1}</span>&nbsp;&nbsp;
    ...
    const CustomerDetails = () => {
        ...
        return (
            <Paper>
                <Typography variant='h4' className={styleClasses.invoice}>Invoice To</Typography>
                ...
                >
                    {
                        customers.map((customer) => {
                            return (
                                <MenuItem key={customer.code} value={customer.code} id={customer.code}>{customer.name}</MenuItem>
                            )
                        })
                    }
                </Select>
                        ...
                        !customers.length ? null : customers.map((customer) => {
                            return (
                                customer.code == selectedCustomer ?
                                    <Card key={customer.code} className={styleClasses.customerDetails}>
                                        <span className={styleClasses.data}><AccountCircle />{customer.name}</span><br />
                                        <span className={styleClasses.data}><Home />{customer.address}</span><br />
                                        <span className={styleClasses.data}><Call />{customer.contact1}</span><br />
                                        <span className={styleClasses.data}>{customer.gst}</span><br />
                                        <span className={styleClasses.data}><Place />{findState(customer.stateCode).name}</span><br />
                                    </Card>
    ...
    return (
        <Paper className={styleClasses.mainContainer}>
            <Card className={styleClasses.headingsContent}>
                <Typography variant='h3' className={styleClasses.heading}>Invoices Generate</Typography>
                <Button className={styleClasses.button} variant='contained' color='primary'><Delete /></Button>
            </Card>
            ...
            <Card className={styleClasses.innerContainer}>
                <Card className={styleClasses.rightSideContainer}>
                    <Typography variant='h4' className={styleClasses.invoice}>Invoice Number</Typography>
